  ...
  return (
    <Card>
        ...
        <table className="table-auto w-full">
          ...
          <tbody className="mt-4">
            <tr>
              ...
              <td className="text-end">{priceFormat(pkg.price, false)}</td>
              <td className="text-end">{quantity}</td>
              <td className="text-end">{priceFormat(total(), false)}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <Title order={3} className="text-end">
        รวมเป็นเงินทั้งหมด
        <span className="text-primary mx-2">{priceFormat(total(), false)}</span>
        บาท
      </Title>
